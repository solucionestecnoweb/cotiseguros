...
class AccountMove(models.Model):
    ...
    #invoice_number_next = fields.Char(default='300')
    #os_currency_rate = fields.Float(default='20')
    ...
    def consiliacion_faltante(self):
        ...
        if self.payment_ids:
            for line_pago in self.payment_ids:
                ...
                for det in line_pago.payment_id.move_id:
                    if det.line_ids:
                        for dett in det.line_ids:
                            if dett.account_id.id==cuenta_anticipo.id:
                                move_line_1=dett
    ...
    def valida_igtf(self):
        for selff in self:
            if selff.state!='draft':
                if selff.usar_anticipo=='si':
                    if selff.payment_ids:
                        for det in selff.payment_ids:
                            if det.payment_id.payment_method_line_id.payment_method_id.calculate_wh_itf==True:
                                if selff.currency_id.id==det.currency_id.id:
                                    monto_base=det.payment_id.amount
                                if selff.currency_id.id!=det.currency_id.id:
                                    if det.currency_id.id==selff.company_id.currency_id.id:
                                        monto_base=det.payment_id.amount
                                    else:
                                        monto_base=det.payment_id.amount*selff.os_currency_rate
                                monto_base_usd=monto_base/selff.os_currency_rate
                                tasa=selff.os_currency_rate
                                asiento_igtf=det.payment_id.asiento_cobro_igtf.id
                                metodo_pago=det.payment_id.payment_method_line_id.payment_method_id.id
                                porcentaje=det.payment_id.payment_method_line_id.payment_method_id.wh_porcentage
                                retencion=monto_base*porcentaje/100
                                vols={
                                'move_id':selff.id,
                                'asiento_igtf':asiento_igtf,#pago_move_id.asiento_cobro_igtf.id,
                                'metodo_pago':metodo_pago,#pago_move_id.payment_method_id.id,
                                'monto_base_usd':monto_base_usd,
                                'tasa':tasa,
                                'monto_base':monto_base,
                                'porcentaje':porcentaje,#pago_move_id.payment_method_id.wh_porcentage,
                                'monto_ret':retencion,#retencion,
                                }
                                selff.env['account.payment.igtf'].create(vols)
    ...
    def action_post(self):
        ...
        if self.usar_anticipo=='si':
            self.crea_asiento_anticipo_v3()
            #self.asocia_anticipo_fact()


    """@api.constrains('amount_residual')
    def asocia_anticipo_fact(self):
        for selff in self:
            if selff.payment_ids:
                for det in selff.payment_ids:
                    det.payment_id.move_id=selff.id"""
        ...
        if not self.payment_ids:
            ...
            for det_anti in self.payment_ids:
                if det_anti.confirmado=='no':
                    ...
                    fecha_doc=det_anti.payment_id.date
                    #fecha_doc=self.date
                    if det_anti.payment_id.currency_id.id!=self.env.company.currency_id.id:
                        ...
                        resultado=valores*det_anti.payment_id.os_currency_rate
                        valores=resultado
                    ...
                    value = {
                        ...
                        'partner_id': self.partner_id.id,
                        #'partner_id': 45,
                        'ref': "Pago Anticipo Factura Nro: %s " % (self.invoice_number_next),
                        'custom_rate':True,
                        'os_currency_rate':det_anti.payment_id.os_currency_rate,
                        'amount_residual':0.0,
                    ...
                    value = {
                        'name': nombre_anti,
                        'ref' : "Anticipo Documento Nro: %s " % (self.invoice_number_next),
...
class AccountPaymentAnticipo(models.Model):
    ...
    tipo = fields.Selection(related='move_id.move_type',store=True)
